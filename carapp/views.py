...
def update_profile(request):
   ...
   if request.method == 'POST':
       if Profile.objects.filter(user_id=current_user).exists():
           form = ProfileForm(request.POST, request.FILES,
                              instance=Profile.objects.get(user_id=current_user))
       else:
           form = ProfileForm(request.POST, request.FILES)
       if form.is_valid():
         profile = form.save(commit=False)
...
def shareholder(request):
    ...
    if shareholder is None:
        message = "you are not registered as a shareholder"
        print(message)
...
# 	return render(request, 'spare/spare.html')
...
def Contact(request):
    ...
    send_mail('New Enquiry', message, sender_email, ['uwizelaetitia@gmail.com'])
    if request.method == 'POST':
